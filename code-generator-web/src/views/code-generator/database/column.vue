...
<script>
  ...
  import { generateCode,dl } from '@/api/code-generator/project'
  ...
  methods: {
    ...
    handleGenerate() {
      this.stamp="t"+new Date().getTime();
      // 可检索字段
      this.tableInfo.queryColumns = this.queryColumns;
      generateCode(this.tableInfo, this.packageConfig,this.stamp).then(response => {
          if (response.code == 200) {
            console.log(this.stamp);

            window.open("http://" + window.location.host + "/api/code/project/download?stamp="+this.stamp , '_blank')
          } else {
              this.submitFail(response.message);
          }
        }).catch(err => {
