...
import com.zhengqing.utils.FastDfsApiUtils;
import com.zhengqing.utils.FileUtils;
import com.zhengqing.utils.ZipFileUtils;
...
//        result = result.stream().collect( Collectors.collectingAndThen( Collectors.toCollection( () -> new TreeSet<>( Comparator.comparing( Project::getName ) ) ), ArrayList::new ) );
    ...
    public Integer saveProject(Project para) {
        ...
        if ( para.getId() != null ) {
            ...
            projectPackage.setName( para.getPackageName());
    ...
    public String generateCode(CodeGenerateInput input) throws IOException {
        ...
        generate( database, input.getTableInfo(), input.getPackageConfig(), tableConfig ,input.getStamp() );
        return zipCode(input.getStamp());
    }
     ...
     */
    private void generate(Database database, TableInfo tableInfo, Map<String,String> packageConfig, TableConfig tableConfig ,String stamp) {
        ...
        // 生成code所在位置
        gc.setOutputDir(Constants.PROJECT_ROOT_DIRECTORY + "/document/code_"+stamp);
        // 是否覆盖同名文件，默认是false
    ...
    private String zipCode2(){
        // 将code目录以及其目录下的所有文件目录打包到/document/目录下的code.zip文件中 true:表示带目录显示
        File zipFile = ZipUtil.zip(Constants.PROJECT_ROOT_DIRECTORY + "/document/code", Constants.PROJECT_ROOT_DIRECTORY + "/document/code.zip", true);
        // 删除目录 -> 保证下次生成代码的时候不会累计上次留下的代码
        FileUtils.DeleteFolder(Constants.PROJECT_ROOT_DIRECTORY + "/document/code");
        // 1、采用腾讯oss对象储存下载
//        return cosFileService.uploadFile("code.zip", zipFile);// "http://jxi-1251616562.cos.ap-chengdu.myqcloud.com/code.zip?sign=q-sign-algorithm%3Dsha1%26q-ak%3DAKIDoTv9B0wzA7rNgmJMr8B1d2ECwTmrjNo1%26q-sign-time%3D1561101804%3B1561104804%26q-key-time%3D1561101804%3B1561104804%26q-header-list%3D%26q-url-param-list%3D%26q-signature%3Db6bf47b4e6cbde71df6b064e289b0d00f6150823";
        ...
        return FastDfsApiUtils.upload(Constants.PROJECT_ROOT_DIRECTORY + "/document/code.zip", "zip");
    }

    /**
     * 下载生成好的代码 - 原生
     */
    private String zipCode(String stamp) {
        try {
            String outZipFileName = Constants.PROJECT_ROOT_DIRECTORY + "/document/code_"+stamp+".zip";
            ...
            String fileName = Constants.PROJECT_ROOT_DIRECTORY + "/document/code_"+stamp;
